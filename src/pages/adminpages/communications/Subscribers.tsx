...
export default function SubscribersContent({
  ...
  const handleConfirmDelete = async () => {
    if (selectedSubscriberId) {
      try {
        ...
        setSubscribers(
          subscribers.filter((sub) => sub.id !== selectedSubscriberId)
        );
        setNotification({
          type: "success",
  ...
  const exportSubscribers = async () => {
    try {
      ...
      const subscribersToExport = subscribers.filter(
      ...
      const csvContent = [
        headers.join(","),
        ...subscribersToExport.map(
          (sub) =>
            `"${sub.name || ""}","${sub.email}","${new Date(
              sub.subscribed_at
            ).toLocaleDateString()}"`
        ),
      ].join("\n");
      ...
      link.setAttribute(
        "download",
        `eacna-subscribers-${new Date().toISOString().split("T")[0]}.csv`
      );
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 mt-1">{totalCount} total subscribers</p>
        </div>
