...
export default function AdminPayments({ supabase }: AdminPaymentsProps) {
  ...
  const [typeFilter, setTypeFilter] = useState<'all' | 'application' | 'renewal' | 'upgrade'>('all');
  ...
  const exportPayments = async () => {
    try {
      // Create CSV content
      const headers = ['Transaction ID', 'Member', 'Email', 'Amount', 'Type', 'Method', 'Status', 'Date', 'Verified'];
      const csvContent = [
        headers.join(','),
        ...filteredPayments.map(payment => 
          `"${payment.transaction_id}","${payment.member_name}","${payment.member_email}",${payment.amount},"${payment.payment_type}","${payment.payment_method}","${payment.status}","${formatDate(payment.created_at)}","${payment.verified_at ? formatDate(payment.verified_at) : 'Not verified'}"`
        )
      ].join('\n');
      ...
      link.href = url;
      link.setAttribute('download', `eacna-payments-${new Date().toISOString().split('T')[0]}.csv`);
            ...
            <div className="p-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div>
                      ...
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusClass(selectedPayment.status)}`}>
                  ...
                  <div className="space-y-2">
                    ...
                    {selectedPayment.verified_at && (
                      <div>
                        ...
                        <p className="text-sm">{formatDate(selectedPayment.verified_at)}</p>
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {selectedPayment.renewal_id && (
                      <div>
                        ...
                        <p className="text-sm font-mono">{selectedPayment.renewal_id}</p>
                      </div>
                    )}
                    {selectedPayment.member_id && (
                      <div>
                        <p className="text-xs text-gray-500">Member ID</p>
                        <p className="text-sm font-mono">{selectedPayment.member_id}</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {selectedPayment.notes && (
                <div className="mb-6">
                  ...
                  <p className="text-sm text-gray-700 bg-gray-50 p-3 rounded-md">{selectedPayment.notes}</p>
                </div>
              )}
