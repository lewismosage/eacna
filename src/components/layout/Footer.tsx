...
const Footer = () => {
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (existingSubscribers) {
        setError(
          "You have already subscribed with this email address. Thank you for your interest!"
        );
        return;
      }
  ...
  return (
    <footer className="text-white pt-16 pb-8 bg-purple-950">
      <div className="container-custom">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
          {/* About */}
          <div>
            <Link to="/" className="flex items-center space-x-2 mb-4">
              <img src={eacna} alt="EACNA Logo" className="h-16 w- 14 " />
              <div>
                <span className="text-xl font-bold tracking-tight block text-white">
                  EACNA
                </span>
                <span className="text-xs text-primary-200 tracking-wider block">
          ...
          <div>
            ...
            <form onSubmit={handleSubscribe} className="space-y-2">
              ...
              <button
                ...
                className={`w-full font-medium py-2 px-4 rounded-md transition-colors border border-primary-700 bg-primary-800 text-white focus:outline-none focus:ring-2 focus:ring-primary-400 hover:bg-primary-700 disabled:opacity-60 disabled:cursor-not-allowed ${
                  isSuccess
                    ? "bg-green-500 text-white border-green-600"
                    : isSubmitting
                    ? "bg-primary-800 text-white border-primary-700 cursor-not-allowed opacity-60"
                    : ""
                }`}
              >
                {isSuccess
        ...
        <div className="border-t border-primary-800 pt-8 mt-8">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <p className="text-primary-300 text-sm mb-4 md:mb-0">
              © {new Date().getFullYear()} East African Child Neurology
              Association (EACNA). All rights reserved.
            </p>
